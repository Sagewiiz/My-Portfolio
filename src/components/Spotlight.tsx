...
export default function Spotlight({
  ...
  const launchSelectedApp = () => {
    if (curDetails.type === "app" && !curDetails.link) {
      ...
      if (id === "launchpad") toggleLaunchpad(true);
        ...
        // close launchpad if opening any app via Spotlight
  ...
  const updateHighlight = (prevIndex: number, curIndex: number) => {
    ...
    if (prev) {
      prev.classList.remove(textWhite, textSelected);
      prev.classList.add(textBlack);
    }
    if (cur) {
      cur.classList.remove(textBlack);
      cur.classList.add(textWhite, textSelected);
    }
  };
